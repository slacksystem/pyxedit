...
class XEditBase:
    ...
    def xelib(self):
        ...
        if self.handle and self.handle not in self._handle_layer:
            raise XEditError(
                f"Accessing XEdit object of handle {self.handle} "
    ...
    def type(self):
        ...
        if self.def_type in (
            ...
            if (
                self.def_type == self.DefTypes.Integer
                and self.value_type == self.ValueTypes.Reference
            ):
    ...
    def objectify(self, handle):
        ...
        if generic_obj.element_type in (
            self.ElementTypes.Array,
        ...
        if (
            generic_obj.element_type == self.ElementTypes.SubRecord
            and generic_obj.value_type == self.ValueTypes.Array
        ):
            return XEditArray.from_xedit_object(handle, self)
    ...
    def get(self, path, default=None, ex=False, absolute=False):
        ...
        if handle:
            ...
            if absolute:
                raise XEditError(
                    f"No object can be obtained with absolute "
                    f"path {path} from the root"
                )
            else:
                raise XEditError(
                    f"No object can be obtained at path {path} "
    ...
    def add(self, path):
        ...
        if self.get(path):
            raise XEditError(
                f"Cannot add object at path {path}; an object " f"already exists there"
            )
    ...
    def signature_name(self):
        ...
        return self.xelib.name_from_signature(signature, ex=False) if signature else ""
    ...
    def references(self):
        ...
        for descendant in self.descendants():
            if descendant.is_ref and not descendant.long_path.endswith(
    ...
    def from_xedit_object(cls, handle, xedit_obj, auto_release=True):
        ...
        if not handle or handle not in xedit_obj._xelib._current_handles:
            raise XEditError(
                f"Attempting to create XEdit object from invalid "
